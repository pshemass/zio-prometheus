...
sealed abstract class Metric[A <: Labels : Tag] {
  ...
  private[prometheus] def accessManaged[B](acquire: RealMetric => B)(release: B => Any) =
    ZManaged.accessManaged[Metric](e =>
      ZManaged.makeEffect(acquire(e.get.metric))(release)
    )
...
object Counter {
  ...
  implicit class CounterEmptyLabelOps[M <: Has[_]](val counter: Counter[Labels.Empty.type] {type Metric = M}) extends AnyVal {
  ...
  implicit class CounterNonEmptyLabelOps[M <: Has[_], A <: Labels](val counter: Counter[A] {type Metric = M}) extends AnyVal {
    def inc(labels: A): URIO[M, Unit] = counter.access(_.labels(labels.asSeq: _*).inc()).asInstanceOf[URIO[M, Unit]]

    def inc(value: Double, labels: A): URIO[M, Unit] = counter.access(_.labels(labels.asSeq: _*).inc(value)).asInstanceOf[URIO[M, Unit]]
  }
...
object Gauge {
  ...
  implicit class GaugeEmptyLabelOps[M <: Has[_]](val metric: Gauge[Labels.Empty.type] {type Metric = M}) extends AnyVal {
  ...
  implicit class GaugeNonEmptyLabelOps[M <: Has[_], A <: Labels](val metric: Gauge[A] {type Metric = M}) extends AnyVal {
    ...
    def inc(value: Double, labels: A): URIO[M, Unit] = metric.access(_.labels(labels.asSeq: _*).inc(value)).asInstanceOf[URIO[M, Unit]]

    def dec(value: Double, labels: A): URIO[M, Unit] = metric.access(_.labels(labels.asSeq: _*).dec(value)).asInstanceOf[URIO[M, Unit]]

    def set(value: Double, labels: A): URIO[M, Unit] = metric.access(_.labels(labels.asSeq: _*).set(value)).asInstanceOf[URIO[M, Unit]]
  }
...
object Summary {
  ...
  implicit class SummaryEmptyLabelOps[M <: Has[_]](val metric: Summary[Labels.Empty.type] {type Metric = M}) extends AnyVal {
    final def timer[R <: M, E, B: Tag](zio: ZIO[R, E, B]) = {
      metric.accessManaged(_.startTimer())(_.observeDuration()).ignore.use_(zio)
    }

    final def observe(value: Double) =
      metric.access(_.observe(value))
  }

  implicit class SummaryNonEmptyLabelOps[M <: Has[_], A <: Labels](val metric: Summary[A] {type Metric = M}) extends AnyVal {
    final def timer[R <: M, E, B: Tag](zio: ZIO[R, E, B], labels: A) = {
      metric.accessManaged(_.labels(labels.asSeq: _*).startTimer())(_.observeDuration()).ignore.use_(zio)
    }

    final def observe(value: Double, labels: A) =
      metric.access(_.labels(labels.asSeq: _*).observe(value))
  }
...
object Histogram {
  ...
  implicit class HistogramEmptyLabelOps[M <: Has[_]](val metric: Histogram[Labels.Empty.type] {type Metric = M}) extends AnyVal {
    final def timer[R <: M, E, B: Tag](zio: ZIO[R, E, B]) = {
      metric.accessManaged(_.startTimer())(_.observeDuration()).ignore.use_(zio)
    }
  ...
  implicit class HistogramNonEmptyLabelOps[M <: Has[_], A <: Labels](val metric: Histogram[A] {type Metric = M}) extends AnyVal {
    final def timer[R <: M, E, B: Tag](zio: ZIO[R, E, B], labels: A) = {
      metric.accessManaged(_.labels(labels.asSeq: _*).startTimer())(_.observeDuration()).ignore.use_(zio)
    }
